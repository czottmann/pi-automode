...
- **Classifier output budgets** — Send each classifier stage the amount its context-fit check already reserves: the answer allowance plus the #51 reasoning reserve. At an explicit reasoning level this raises the shared output ceiling. It does not keep hidden reasoning out of the decision tool call. After a length stop, the detailed retry uses the model output limit or the estimated context room, whichever is smaller, and never below the first ceiling; when it cannot exceed the first ceiling, no retry is sent and the action fails closed.
